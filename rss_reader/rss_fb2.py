...
def save_img(url, number):
    ...
    logging.info("Downloading image for fb2")
...
def remove_image(i):
    ...
    try:
        logging.info("Deleting image for fb2")
...
def encode_image(path):
    ...
    with open(path, "rb") as im:
                    ...
                    picture_str = str(encoded_image)
                    picture_str = picture_str[2:len(picture_str)-1]
                    return picture_str
    ...
    with open("News.fb2", "w") as output:
        ...
        for i in range(length):
            ...
            if news[i].url_images != [''] and news[i].url_images != None and news[i].url_images != []:
                ...
                file_name = "_" + str(i)
                fn = os.path.join(os.path.dirname(os.path.abspath(__file__)), file_name)
                output.write('<binary id="_'+str(i)+'" content-type="image/jpeg"> ' + encode_image(fn) + ' </binary>\n')
                remove_image(i)
